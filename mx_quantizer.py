...
def _quantize_elemwise_core(A, bits, exp_bits, max_norm, round="nearest", saturate_normals=False, allow_denorm=True):
    ...
    # bits: number of mantissa bits. Includes sign bit and implicit one for floats
...
if __name__ ==  '__main__':
    ...
    # A = torch.arange(2**-9, 2**-6 * 0.875, 0.00042, dtype=torch.float32)
    ...
    # qdq_A = _quantize_elemwise_core(A, bits=5, exp_bits=4, max_norm=448., round="nearest", saturate_normals=True, allow_denorm=True)
    qdq_A_scaled = quantize_mx_op(A, elem_format, round_method, block_size, axes=axes)

    print ('mx_fpx:', qdq_A_scaled)
    print ('A_e4m3:', A_e4m3)
    print ('diff between A_e4m3 and mx_fpx:', torch.abs(A - qdq_A_scaled).max())